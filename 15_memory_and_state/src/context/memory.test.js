...
import { emptyTask, stageOf } from "./taskState.js";
...
test("the extractor is shown what is stored, with its values", async () => {
  ...
  // model silently drop what it did not restate. Two exchanges plus the reply
  // above them is five messages at the very most, whatever the history.
  for (const call of extractor.calls) assert.ok(call.exchange.length <= 5);
});

test("the extractor reads two exchanges, so one turn it could not read is not the end of it", async () => {
  const { strategy, extractor } = strategyWith({});
  await converse(strategy, ["first", "second", "third"]);

  // The newest exchange, the one before it, and the reply above that. The
  // middle pair is the point: when the call on turn two failed — a timeout, a
  // patch that came back as prose — turn three is the only chance the store
  // will ever get to read it, and with a one-exchange window there was none.
  assert.deepEqual(extractor.calls[2].exchange.map((message) => message.content), [
    "ok",
    "second",
    "ok",
    "third",
  ...
  // It opens on a reply rather than on a user message, and that is deliberate:
  // "yes, do that" at the top of the window needs the question it answers.
  assert.equal(extractor.calls[2].exchange[0].role, "assistant");
});

test("the window is a knob, and at one exchange it is what it always was", async () => {
  const { strategy, extractor } = strategyWith({ extractionExchanges: 1 });
  await converse(strategy, ["first", "second", "third"]);

  assert.deepEqual(extractor.calls[2].exchange.map((message) => message.content), ["ok", "third"]);
});
...
test("the fold summarises what was said and never what the task produced", async () => {
  // The goal is not decoration here: it is what the crossing out of planning is
  // guarded on, and without one there is no work for the ceiling to protect.
  const { strategy, summarizer } = strategyWith({ window: 4, ops: [[{ op: "set", key: "goal", value: "write the thing" }]] });

  // Two exchanges of planning, then the crossing. Everything after it is the
  // work, and the brief now stands in for everything before it.
  const planning = await converse(strategy, ["what I want", "and this too"]);
  const crossed = await toExecution(strategy, { state: planning.state, history: planning.history });
  assert.equal(stageOf(crossed.state.task), "execution");

  const working = await converse(strategy, ["do it", "carry on", "and again", "once more"], {
    from: { history: planning.history, state: crossed.state },
  });

  // The fold ran once, up to the crossing, and then stopped there. Without the
  // ceiling it would have gone on eating the exchanges the work is in — which
  // is how `validation` ended up asking to be shown work it had produced two
  // turns earlier.
  assert.equal(summarizer.calls.length, 1);
  assert.equal(working.state.digestThrough, 4);
  assert.ok(working.turns.at(-1).meta.note.includes("stops where this task's work begins"));

  // ...and the first exchange of the work is still on the wire, verbatim.
  assert.ok(working.turns.at(-1).messages.some((message) => message.content === "do it"));
});

test("closing the task releases the fold", async () => {
  const { strategy, summarizer } = strategyWith({ window: 4, ops: [[{ op: "set", key: "goal", value: "write the thing" }]] });
  const planning = await converse(strategy, ["what I want", "and this too"]);
  const crossed = await toExecution(strategy, { state: planning.state, history: planning.history });
  const working = await converse(strategy, ["do it", "carry on", "and again", "once more"], {
  ...
  let current = working.state;
  for (const to of ["validation", "done"]) {
    const moved = await strategy.transition({ state: current, to, history: working.history });
    assert.equal(moved.ok, true, `${to} was refused`);
    current = moved.state;
  }
  assert.equal(stageOf(current.task), "done");

  // Finished work is no longer the thing being checked, so the digest may have
  // it. The ceiling is a property of the live task and it retires with it.
  const after = await converse(strategy, ["what next?"], {
    from: { history: working.history, state: current },
  });
  assert.equal(summarizer.calls.length, 2);
  assert.ok(after.state.digestThrough > 4);
});

test("a digest that already swallowed the work puts it back on the wire", async () => {
  const { strategy } = strategyWith({ window: 10 });
  const history = [];
  for (const word of ["one", "two", "three", "four", "five"]) {
    history.push({ role: "user", content: word });
    history.push({ role: "assistant", content: `reply to ${word}` });
  }
  history.push({ role: "user", content: "check the work against the goal" });

  // State as the missing ceiling left it: the crossing on turn three, the
  // digest's edge two messages past it, and the work inside the gap. The
  // ceiling cannot un-summarise that, so the floor is clamped instead and
  // those exchanges are sent as well as summarised. Paying twice is the cheap
  // half of this; answering "I have not seen the work" is the expensive half.
  const state = {
    ...strategy.emptyState(),
    task: {
      ...emptyTask(),
      transitions: [{ from: "planning", to: "execution", at: null, by: "user", turn: 3, reason: null }],
    },
    brief: { text: "THE BRIEF", status: "accepted", turn: 3, through: 6, edited: false, open: [] },
    briefThrough: 6,
    digest: "a paragraph that dropped the code",
    digestThrough: 8,
  };

  const built = await strategy.buildPayload({ history, systemPrompt: "persona", state, provider: new StubProvider() });
  assert.equal(built.meta.droppedMessages, 6);
  assert.ok(built.messages.some((message) => message.content === "reply to four"));
});
...
test("the extractor is told to file the questions the assistant asked", async () => {
  ...
  // exchange, which is the only reason the extractor can file it at all. Where
  // in the window it lands is the window's business; that it is in there at
  // all is this rule's.
  assert.ok(extractor.calls[1].exchange.some((message) => message.role === "assistant"));
  ...
  // ...and its mirror, which is what the blanket "record only what the USER
  // established" rule used to forbid. A choice the assistant announced and
  // moved on from had nowhere to go but the `awaiting` op, which is how
  // "I'll use an iterative DFS, the rules forbid recursion" became a question
  // the user was told to confirm. Two shapes, both with keys, and no third.
  assert.match(prompt, /what the assistant \*\*settled\*\*/);
  assert.match(prompt, /is a `decision\.\*`, not a question/);
  assert.doesNotMatch(prompt, /"op":"awaiting"/);
});
...
test("the model's step is written; its stage is only ever a suggestion", async () => {
  const { strategy } = strategyWith({
    ops: [
      [
        { op: "set", key: "goal", value: "migrate billing off Heroku" },
  ...
  // Whose turn it is is the stage's own answer, not the model's.
  assert.deepEqual(lifecycle.expectedAction, { actor: "user", what: "say what the task is, and answer the open questions" });
  ...
  assert.match(turns[0].system, /awaiting: the user — say what the task is/);
});
...
test("a question the reply already answered does not ride into execution", async () => {
  // The dialog this came from: turn three's extraction ran before the reply and
  // invented `open.starting_node`; the reply written a moment later said "the
  // starting node is passed as a parameter — everything's clear, we can switch
  // to execution now". Nothing on the turn path can close that row, because the
  // message that answers it has never been read, so `→ execution` went on
  // warning about a question that was settled two lines above the button.
  const { strategy, briefer } = strategyWith({
    ops: [
      [{ op: "set", key: "goal", value: "Implement a generic DFS" }],
      [
        { op: "set", key: "decision.graph_representation", value: "Map<Int, List<Int>>" },
        { op: "set", key: "open.starting_node", value: "Is the starting node a parameter?" },
      ],
    ],
  });
  // The briefer has read the reply, so it reports one of the two as open and
  // says nothing about the other.
  briefer.open = [{ key: "open.error_handling", question: "What happens on malformed input?" }];

  const provider = new StubProvider();
  const talk = await converse(strategy, ["build me a DFS", "whatever is easier"], { provider });
  assert.equal(talk.state.working["open.starting_node"].turn, 2);

  // It is still there while the brief is pending, and the warning still reports
  // it — the reconciliation belongs to the accept, not to the draft.
  const written = await strategy.transition({ state: talk.state, to: "execution", history: talk.history, provider });
  assert.equal(written.awaitingBrief, true);
  assert.ok(written.state.working["open.starting_node"]);
  ...
  // The stale row is gone, the one the brief found is written, and the note
  // says both happened: a row that vanished without a word is a row someone
  // goes looking for.
  assert.equal(accepted.ok, true);
  assert.deepEqual(
  ...
  assert.match(accepted.note, /1 open question recorded/);
  assert.match(accepted.note, /1 the brief found already answered/);

  // Nothing else in working memory is the brief's business.
  assert.equal(accepted.state.working["decision.graph_representation"].value, "Map<Int, List<Int>>");
  assert.equal(accepted.state.working.goal.value, "Implement a generic DFS");
});

test("the reconciliation only reaches what the brief could actually have read", async () => {
  const { strategy, briefer } = strategyWith({
    ops: [
      [{ op: "set", key: "goal", value: "Implement a generic DFS" }, { op: "set", key: "open.old", value: "Asked long ago?" }],
      [{ op: "set", key: "open.fresh", value: "Asked while the brief sat waiting?" }],
    ],
  });
  briefer.open = [];
  ...
  const written = await strategy.transition({ state: talk.state, to: "execution", history: talk.history, provider });
  assert.equal(written.state.brief.turn, 1);

  // A turn taken while the brief is pending. Its question was never in front of
  // the briefer, so an empty `open` list says nothing about it.
  const later = await converse(strategy, ["one more thing"], {
    provider,
    from: { history: talk.history, state: written.state },
  });
  ...
  assert.equal(accepted.ok, true);
  assert.deepEqual(Object.keys(accepted.state.working).filter((key) => key.startsWith("open.")), ["open.fresh"]);
  assert.match(accepted.note, /1 the brief found already answered/);
});

test("a reply that answers an open question closes it, on the turn it answers it", async () => {
  const { strategy } = strategyWith({
    ops: [[{ op: "set", key: "goal", value: "Implement a generic DFS" }, { op: "set", key: "open.starting_node", value: "Is the starting node a parameter?" }]],
  });
  const provider = new StubProvider();
  const history = [{ role: "user", content: "build me a DFS" }];
  const built = await strategy.buildPayload({ history, systemPrompt: "persona", state: strategy.emptyState(), provider });

  // The ask rides in the block that holds the question, and names the key.
  assert.match(built.system, /CLOSING A QUESTION/);
  assert.match(built.system, /<memory-close>/);

  const reply = [
    "Got it. I'll go with:",
    "- **Starting node:** passed as a parameter",
    "",
    "Everything's clear — we can switch to execution now.",
    '<memory-close>{"key":"open.starting_node","answer":"Passed as a parameter by the caller"}</memory-close>',
  ].join("\n");

  const after = await strategy.afterTurn({ history, state: built.state, reply });

  // The question is gone and the answer is in its place — no second call, and
  // nothing for a person to delete by hand.
  assert.equal(after.state.working["open.starting_node"], undefined);
  assert.equal(after.state.working["decision.starting_node"].value, "Passed as a parameter by the caller");
  assert.equal(after.usage, null, "closing a question is a parse, not a call");

  // ...and the reader never sees the line that did it.
  assert.doesNotMatch(after.reply, /memory-close/);
  assert.match(after.reply, /switch to execution now\.$/);

  // Which is the whole point: the edge that used to warn about it has nothing
  // left to warn about.
  const edge = strategy
    .panel(after.state, { turns: 1 })
    .lifecycle.edges.find((e) => e.to === "execution");
  assert.equal(edge.warning, null);
});

test("the block says nothing about closing when there is nothing open", async () => {
  const { strategy } = strategyWith({ ops: [[{ op: "set", key: "goal", value: "Implement a generic DFS" }]] });
  const { turns } = await converse(strategy, ["build me a DFS"]);
  assert.doesNotMatch(turns[0].system, /CLOSING A QUESTION|memory-close/);
});

test("a close that names nothing, or is not JSON, is reported rather than written", async () => {
  const { strategy } = strategyWith({
    ops: [[{ op: "set", key: "goal", value: "ship it" }, { op: "set", key: "open.region", value: "Which region?" }]],
  });
  const provider = new StubProvider();
  const history = [{ role: "user", content: "ship it" }];
  const built = await strategy.buildPayload({ history, systemPrompt: "persona", state: strategy.emptyState(), provider });
  ...
  const after = await strategy.afterTurn({
    ...
    reply: [
      "Frankfurt it is.",
      '<memory-close>{"key":"open.nothing_like_this","answer":"whatever"}</memory-close>',
      "<memory-close>not json at all</memory-close>",
    ].join("\n"),
  });

  // Nothing written, both fences gone from the text, and both said out loud —
  // a close that quietly did nothing is indistinguishable from the model never
  // having emitted one.
  assert.ok(after.state.working["open.region"], "a real question was closed by a bogus close");
  assert.ok(!Object.keys(after.state.working).some((key) => key.startsWith("decision.")));
  assert.doesNotMatch(after.reply, /memory-close|not json/);
  const reasons = after.state.discarded.map((row) => row.reason);
  assert.ok(reasons.some((reason) => /no open question by that key/.test(reason)));
  assert.ok(reasons.some((reason) => /not JSON/.test(reason)));
});
...
test("what the brief leaves open becomes keys, and the accept is the write", async () => {
  ...
  assert.match(done.warning ?? "", /2 questions are recorded as open/);
