...
import { exchangeStarts, foldTo, snapToUserMessage, toWire, windowStart } from "./boundaries.js";
import {
  KEY_PATTERN,
  NAMESPACES,
  PROPOSABLE,
...
/**
 * The fence the answering model closes a question with. Deliberately ugly and
 * deliberately not JSON-only: it has to be findable in prose with a regex, and
 * it has to be obvious in a transcript when the strip fails, rather than
 * looking like something the assistant meant to say.
 */
const CLOSE_OPEN = "<memory-close>";
const CLOSE_CLOSE = "</memory-close>";
/** How many a single reply may close. More than this is a reply gone wrong. */
const MAX_CLOSES = 6;

/**
 * **How many exchanges the extractor reads.**
 *
 * One was the original answer and it was very nearly enough: extraction runs
 * before the reply, over the newest user message with the reply above it for
 * context, so nothing the user says is ever read late. What one exchange has
 * no way to recover from is a turn that *failed* — a timed-out extractor, a
 * patch that came back as prose — because the exchange it could not read is
 * never in the window again. Two means a dropped turn heals itself on the
 * next one, and that a reference reaching back further than the last question
 * ("no, the other one") has something to reach.
 *
 * It is not what closes the one-turn lag on the assistant's own reply, and
 * widening it further would not either: the reply being written right now
 * cannot be in any window, however long. See `panel()`'s `reads`.
 */
const EXTRACTION_EXCHANGES = 2;
...
export function workingBlock(working, task = null, { invariants = 0 } = {}) {
  ...
  return [
    ...
    ...stage,
    ...lines,
    ...closingLines(working),
    "</working>",
  ].join("\n");
}

/**
 * **The one thing the answering model is asked to write down, and why it is
 * asked at all.**
 *
 * Extraction runs before the reply — that is what lets a preference stated in
 * this message shape the answer to this message — and the cost is that nothing
 * reads the reply until the next turn. Usually that is invisible. There is one
 * shape where it is not, and it is the shape planning produces constantly: the
 * reply *answers an open question itself*. "The starting node is passed as a
 * parameter — everything's clear, we can switch to execution now." The question
 * is settled in the same breath, and memory goes on asserting it is open, the
 * edge warning goes on reporting it, and the only way out was for a human to
 * delete a row that was already answered in front of them.
 *
 * No window fixes that, because the message doing the answering is the one
 * being written. Reading it costs a second call — and this is the one way to
 * read it without making one: **the call that writes the answer writes the
 * close.** It is the smallest possible version of that idea. Not a patch, not
 * the memory format, not anything about the user: the keys it is allowed to
 * name are the `open.*` rows printed directly above, and the only thing it may
 * say about them is *this one is answered, and here is the answer*. Everything
 * else in memory stays the extractor's job, at temperature 0, on its own call.
 *
 * Emitted only when there is something to close, so a conversation with no open
 * questions pays nothing for this and is never told about it.
 */
function closingLines(working) {
  const open = orderKeys(working).filter((key) => key === "open" || key.startsWith("open."));
  if (!open.length) return [];
  return [
    "",
    `CLOSING A QUESTION. Exactly ${open.length === 1 ? "one of the lines above is an open question" : `${open.length} of the lines above are open questions`}` +
      " — a thing nobody has answered yet. If your reply answers one, say so in a single line at the very" +
      " end of the message, after everything else, in this form and nothing else:",
    `${CLOSE_OPEN}{"key":"open.<the key above>","answer":"<what the answer is, under 20 words>"}${CLOSE_CLOSE}`,
    "One per question answered, on one line each. The user never sees these lines; they are how the answer" +
      " reaches memory, and without them the question stays on the list and gets asked again. Only the keys" +
      " listed above, only when the answer is actually settled in this reply, and never for a question you" +
      " are still asking. Nothing else goes in them, and nothing else is written this way.",
  ];
}
...
const EXTRACTOR_PROMPT = [
  ...
  "and the last few messages of it, oldest first.",
  "",
  "Return ONLY the changes the newest of those messages makes: a JSON array of",
  ...
  "key-value pairs. There is no op for whose turn it is: an unanswered question is an",
  "`open.*` and a stated assumption is a `decision.*`, and both of those are keys:",
  ...
  "- **The newest message is the one you are reporting on.** The ones above it are context.",
  "  They are what makes 'yes', 'the second one' and 'no, the other way' readable, and they",
  "  are a second chance at something an earlier turn failed to record. What they must not",
  "  do is make you restate a key that is already stored with the right value, or bring back",
  "  an `open.*` the store no longer has: a question that was answered and deleted stays",
  "  deleted, however far up it is still visible being asked.",
  ...
  "- Record what the USER established — and what the assistant **settled**. Its opinions,",
  "  its examples and the options it is still weighing are not facts. But a choice it",
  "  announced and moved on from is: `\"the rules forbid recursion, so I'll use an iterative",
  "  DFS with an explicit stack\"` is a `decision.*`, not a question, and not something to",
  "  file as waiting on the user. The user corrects it by saying so, and then you `set` it",
  "  again — that is what makes it safe to record. Two shapes and no third: something",
  "  nobody has answered is an `open.*`, something that has been settled by either of them",
  "  is a `decision.*`.",
...
export class MemoryStrategy extends ContextStrategy {
   ...
   * @param {number} [params.extractionExchanges] - How much of the tail the
   *   extractor reads. See `EXTRACTION_EXCHANGES`.
  ...
  constructor({
    ...
    maxWorking = 40,
    foldSize = null,
    extractionExchanges = EXTRACTION_EXCHANGES,
    maxTokens = 300,
    model,
    user = DEFAULT_USER,
    ...
    this.extractionExchanges = extractionExchanges;
    this.maxTokens = maxTokens;
    this.model = model;
    this.user = user;
    /**
  ...
  async buildPayload({ history, systemPrompt, state, provider, model }) {
    ...
    // ...and neither floor may reach into the work itself, for the reason the
    // fold may not (see `#fold`). This is the same ceiling applied one step
    // later, and it is what repairs a state whose digest edge had already run
    // past the crossing before the rule existed: those exchanges go back on
    // the wire verbatim, summarised *and* sent, because a paragraph about the
    // work is not a substitute for the work.
    const floor = Math.min(Math.max(next.digestThrough, next.briefThrough), workStartsAt(history, next.task));
  ...
  /**
   * **The reply, read — with no call to read it.**
   *
   * The one hook the other four strategies leave as a no-op, used here for the
   * one thing that genuinely cannot be known before the answer exists: whether
   * the answer settled a question memory has recorded as open. The reply was
   * asked to say so on its way out (see `closingLines`), so this is a parse of
   * text already paid for, and it reports no usage because it spent nothing.
   *
   * Everything it produces goes through `applyOps` with `origin: "model"`, so
   * the frozen goal, the declared profile entries and the invariant subjects
   * are all protected here exactly as they are during extraction. The one
   * liberty taken is the key the answer lands on: `open.starting_node` becomes
   * `decision.starting_node`, by the router's own rule that where a fact lives
   * is a lookup and never a judgement. Both are working-layer and both die with
   * the task, so a fact that should have been a `finding.*` is filed one drawer
   * over and costs nothing — where the extractor, on its own call, can still
   * correct it next turn.
   *
   * It also hands back the reply with the fences taken out, which is the only
   * reason the Agent's `afterTurn` contract returns a reply at all.
   */
  async afterTurn({ history = [], state, reply }) {
    const { text, closes, rejected } = readCloses(reply);
    if (!closes.length && !rejected.length) return { state, usage: null, ms: 0, reply: text };

    const turn = exchangeStarts(history).length;
    const current = normaliseState(state);
    const invariants = await this.#loadInvariants();

    // Only the questions that are actually on the list. A close naming
    // something else is a close of nothing, and saying so is cheaper than
    // letting it write a key nobody asked for.
    const ops = [];
    const unknown = [];
    for (const close of closes) {
      if (!current.working[close.key]) {
        unknown.push({ key: close.key, op: "close", value: close.answer, reason: "no open question by that key" });
        continue;
      }
      ops.push({ op: "set", key: `decision.${close.key.slice("open.".length)}`, value: close.answer });
      ops.push({ op: "delete", key: close.key, from: "working" });
    }

    const applied = ops.length
      ? applyOps({ working: current.working, profile: { entries: {}, nextId: 1 }, invariants }, ops, {
          turn,
          maxWorking: this.maxWorking,
          allow: ["set", "delete"],
          origin: "model",
          frozen: frozenKeys(current.task),
        })
      : null;
    ...
    return {
      state: {
        ...current,
        working: applied?.working ?? current.working,
        proposals: withCorrections(current.proposals, applied?.contested ?? [], turn),
        discarded: mergeDiscarded(current.discarded, [
          ...rejected.map((row) => ({ ...row, turn })),
          ...unknown.map((row) => ({ ...row, turn })),
          ...(applied?.discarded ?? []),
        ]),
      },
      usage: null,
  ...
  panel(state, { turns = Infinity, invariants = null, profile: liveProfile = null, user = null, project = null } = {}) {
    ...
    return {
      ...
      lifecycle: {
        ...
        stages: [...STAGES],
        step: task.step,
        expectedAction: expectedActionOf(task),
  ...
  async answerBrief({ state, history = [], action, text, drop = [] }) {
    ...
    /**
     * **The brief is where a stale `open.*` row dies.**
     *
     * Extraction runs before the reply, so a question the assistant opened and
     * then answered in the very same turn — *"the starting node is passed as a
     * parameter… everything's clear, we can switch to execution now"* — is
     * still a row in working memory, and the edge warning is still reporting
     * it. Nothing on the turn path can close it: the message that answers it
     * has never been read.
     *
     * The `Briefer` has read it. It is handed the whole planning conversation,
     * every message including that reply, and it reports what is *still* open
     * — which makes its list better informed than the stored rows, and this
     * the one moment in the task where the two can be reconciled without
     * paying for another call.
     *
     * Three rules keep that from eating anything real:
     *
     *   - **`open.*` only.** No other namespace is touched, whatever the brief
     *     did or did not mention.
     *   - **Only rows the `Briefer` could have seen** — stamped on or before
     *     the turn the brief was written. A question recorded by a turn taken
     *     while the brief sat pending was not in front of it, and survives.
     *   - **Only after a brief that was actually written.** A failed call
     *     leaves nothing pending, so there is no accept to get here at all;
     *     an empty `open` list therefore means *I read all of planning and
     *     nothing is unanswered*, which is exactly the claim being trusted.
     *
     * A row the person dropped with `×` is not resurrected by this and not
     * counted as settled by it either: they said to leave it out, and out is
     * out.
     */
    const reported = new Set(questions.map((row) => row.key));
    const settled = Object.keys(current.working).filter(
      (key) =>
        (key === "open" || key.startsWith("open.")) &&
        !reported.has(key) &&
        !unwanted.has(key) &&
        (current.working[key].turn ?? 0) <= (current.brief.turn ?? 0)
    );

    const ops = [
      ...questions.map((row) => ({ op: "set", key: row.key, value: row.question })),
      ...settled.map((key) => ({ op: "delete", key, from: "working" })),
    ];
    const applied = ops.length
      ? applyOps({ working: current.working, profile: { entries: {}, nextId: 1 } }, ops, {
          turn,
          maxWorking: this.maxWorking,
          allow: ["set", "delete"],
          // **The brief is the record**, which is what buys the exemption from
          // *an answered question must be replaced, not just removed*. That
          // rule is there because an `open.*` key is very often the only place
          // a fact was ever written down, and a model closing one erases the
          // subject. Here the answer is in the prose the person has just read
          // and approved, and that prose is on the wire under `<brief>` from
          // the next turn on — so the subject survives the key.
          unpairedCloses: true,
          origin: "person",
        })
      : null;
    ...
    return {
      ...
      note:
        ...
        (settled.length ? ` · ${settled.length} the brief found already answered` : "") +
  ...
  async #extract({ history, state, profile, invariants, provider, model, turn }) {
    const exchange = recentMessages(history, this.extractionExchanges);
  ...
  async #fold({ history, state, provider, model }) {
    ...
    // **The digest may summarise what was said. It may never summarise what
    // the task in hand produced.**
    //
    // The fold's own rule is that the oldest exchanges are the least useful,
    // and inside planning that holds. Past the crossing it is exactly wrong:
    // what the work produced is the *oldest* thing in execution, and
    // `validation` exists to check that against the goal. Folded, it comes
    // back as prose with the code dropped out of it, and the next turn reports
    // in good faith that it has never seen the work — which is what this
    // ceiling is here to have made impossible.
    //
    // Nothing is paid for twice: the brief already stands in for everything
    // before the crossing, so the ceiling only ever holds the fold at a line
    // the brief has already reached. What it costs is that a long-running
    // task's own exchanges stay verbatim, and the release is `→ done` — the
    // point at which the work stops being the thing that has to be checked.
    const ceiling = workStartsAt(history, state.task);
    const wanted = foldTo(history, state.digestThrough, this.fold);
    const boundary = Math.min(wanted, ceiling);
    if (boundary <= state.digestThrough) {
      // Said out loud only when the ceiling is what stopped it. A fold with
      // nothing to do is not news; one being held back every turn by the work
      // in hand is the sort of thing you want to read in the note rather than
      // deduce from a token count.
      const note = wanted > ceiling ? "the digest stops where this task's work begins" : "";
      return { state, note, overhead: { ...NO_OVERHEAD } };
    }
...
/**
 * **The tail of the conversation the extractor reads**: the last `exchanges`
 * exchanges, plus the reply immediately above them.
 *
 * That leading reply is not decoration. Without it "yes, do that" and "the
 * second one" say nothing, and a question the assistant asked has nothing to
 * be *unanswered of* — which is the one thing in the assistant's own message
 * the extractor is asked to record.
 *
 * Still a window, and deliberately a short one: the whole history would make
 * the call a regenerative prompt, which is what makes a model silently drop
 * everything it did not happen to restate. At `exchanges = 1` this is exactly
 * what it was before — the newest user message and the reply above it.
 */
/**
 * **Where the work in hand starts, as an index into the history** — or
 * `history.length` when nothing is being protected.
 *
 * `workStartedOn` gives the turn the task left planning; the first message of
 * the *next* exchange is the first one that belongs to the work. A task still
 * in planning, one already `done`, and one that has never had a transition all
 * return the end of the history, which makes every caller's `Math.min` a no-op
 * rather than a special case.
 */
function workStartsAt(history, task) {
  const messages = history ?? [];
  if (stageOf(task) === "done") return messages.length;
  const turn = workStartedOn(task);
  if (turn == null) return messages.length;
  const starts = exchangeStarts(messages);
  return turn < starts.length ? starts[turn] : messages.length;
}

/**
 * **The closes out of a reply, and the reply without them.**
 *
 * Pulled out with a regex over prose rather than parsed as structure, because
 * that is what it is: a line at the end of a message written by the model that
 * wrote the message. Anything wearing the fence is stripped whether or not it
 * parses — a reader should never see this, least of all a broken one — and what
 * failed to parse comes back as `rejected` so the panel can say so.
 */
function readCloses(reply) {
  const raw = typeof reply === "string" ? reply : "";
  const fence = new RegExp(`${CLOSE_OPEN}([\\s\\S]*?)${CLOSE_CLOSE}`, "g");
  const closes = [];
  const rejected = [];

  for (const match of raw.matchAll(fence)) {
    if (closes.length >= MAX_CLOSES) {
      rejected.push({ key: "(no key)", op: "close", value: "", reason: `more than ${MAX_CLOSES} closes in one reply` });
      break;
    }
    let parsed;
    try {
      ...
      rejected.push({ key: "(no key)", op: "close", value: String(match[1]).trim().slice(0, 120), reason: "a close that is not JSON" });
      continue;
    }
    const key = String(parsed?.key ?? "").trim().toLowerCase();
    const answer = String(parsed?.answer ?? parsed?.value ?? "").replace(/\s+/g, " ").trim();
    if (!key.startsWith("open.") || key.length <= "open.".length || !answer) {
      rejected.push({ key: key || "(no key)", op: "close", value: answer.slice(0, 120), reason: "a close needs an open.<subject> key and an answer" });
      continue;
    }
    closes.push({ key, answer: answer.slice(0, MAX_VALUE_LENGTH) });
  }

  // The fence never survives into the transcript, parsed or not.
  const text = raw.replace(fence, "").replace(new RegExp(`${CLOSE_OPEN}|${CLOSE_CLOSE}`, "g"), "").replace(/\n{3,}/g, "\n\n").trim();
  return { text: text || raw.trim(), closes, rejected };
}

function recentMessages(history, exchanges = EXTRACTION_EXCHANGES) {
  const messages = history ?? [];
  if (!messages.length) return [];
  const start = windowStart(messages, exchanges);
  const from = start > 0 && messages[start - 1].role === "assistant" ? start - 1 : start;
...
function extractionPrompt(stored, exchange, invariants = []) {
  return [
    ...
    ...(invariants.length
      ...
      : []),
    "",
    "THE LAST FEW MESSAGES — oldest first. The final one is what was just sent, and it is",
    "the one you are reporting changes for:",
