...
export class ContextStrategy {
   ...
   * @returns {Promise<{ state: object, usage: import("../llm/provider.js").Usage | null, ms: number, reply?: string }>}
   *   `reply` is optional and almost always absent: it is the reply with the
   *   strategy's own bookkeeping taken out of it, for a strategy that asked the
   *   answering model to write something addressed to the store rather than to
   *   the reader. Return nothing and the reply is stored exactly as it arrived.
   */
  // eslint-disable-next-line no-unused-vars
