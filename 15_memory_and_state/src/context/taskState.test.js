...
test("both edges that walk away from an open question warn, and neither blocks", () => {
  ...
  assert.match(started.warning, /1 question is recorded as open/);
...
test("whose turn it is is derived from the stage and cannot go stale", () => {
  // It used to be a stored field a model wrote, and a stale one could only be
  // argued with: no key, no row, no `forget`. Now it is a lookup, so every task
  // in a stage says the same thing and nothing has to be kept fresh.
  assert.deepEqual(expectedActionOf(emptyTask()), DEFAULT_EXPECTED.planning);
  for (const stage of STAGES) {
    const task = stage === "planning" ? emptyTask() : walk(emptyTask(), routeTo(stage));
    assert.deepEqual(expectedActionOf(task), DEFAULT_EXPECTED[stage], stage);
  }

  // Which is also the whole of the warning story: the only thing that can make
  // an edge warn now is an `open.*` key, and a task with none never does.
  const work = { goal: { value: "Ship the thing" } };
  assert.equal(warningFor("planning", "execution", work, emptyTask()), null);
  assert.equal(warningFor("validation", "done", work, walk(emptyTask(), routeTo("validation"))), null);
});

test("the awaiting op is gone, and one that arrives anyway says where it should have gone", () => {
  // Two conversations produced this. In one, the op said `actor: "agent"` in
  // planning, the strip turned that into "▶ let it continue", and pressing it
  // got the deliverable written a stage early. In the other it said
  // `actor: "user"` — "confirm whether to use iterative DFS instead of
  // recursive" — about an assumption the reply had *stated*, citing an
  // invariant the user cannot override anyway. It then sat in the edge warning
  // for three turns, because the field had no key, no row and no way to close.
  //
  // Both meanings have a home with a key. Nobody has answered it: `open.*`.
  // Somebody settled it, either of them: `decision.*`, overwritten when the
  // user says otherwise.
  for (const op of [
    { op: "awaiting", actor: "agent", what: "sketch the iterative DFS implementation" },
    { op: "awaiting", actor: "user", what: "confirm whether to use iterative DFS instead of recursive" },
    { op: "awaiting", what: "no actor at all" },
  ]) {
    for (const stage of STAGES) {
      const task = stage === "planning" ? emptyTask() : walk(emptyTask(), routeTo(stage));
      const applied = applyTaskOps(task, [op], { turn: 3 });

      // Nothing written, whose-turn-it-is still the stage's own answer.
      assert.deepEqual(applied.wrote, [], stage);
      assert.deepEqual(expectedActionOf(applied.task), DEFAULT_EXPECTED[stage], stage);

      // ...and refused *visibly*, naming the two keys it should have been.
      assert.equal(applied.rejected.length, 1, stage);
      assert.equal(applied.rejected[0].op, "awaiting");
      assert.match(applied.rejected[0].reason, /open\.\*.*decision\.\*/);
    }
  }

  // The ops that are still ops are unaffected by its removal.
  const described = applyTaskOps(walk(emptyTask(), ["execution"]), [
    { op: "step", value: "writing the migration script" },
    { op: "awaiting", actor: "user", what: "gone" },
    { op: "stage", to: "validation", reason: "the script is written" },
  ], { turn: 4 });
  assert.equal(described.task.step, "writing the migration script");
  assert.equal(described.suggested, "validation");
  assert.equal(described.rejected.length, 1);
});
...
test("a model may describe the task; it may not change its stage", () => {
  ...
  assert.deepEqual(taskOps.map((op) => op.op), ["step", "stage"]);

  const applied = applyTaskOps(task, taskOps, { turn: 4 });

  // The one descriptive field is written: nobody will click a button to keep a
  // status line fresh, and a stale one is worse than none.
...
test("a transition replaces the step rather than carrying it over", () => {
  ...
  // Whose turn it is needs no resetting — it follows the stage on its own.
  assert.deepEqual(expectedActionOf(moved.task), DEFAULT_EXPECTED.validation);
});
