...
export class Agent {
  ...
  async run(userInput, { branchId = this.#record.activeBranchId } = {}) {
     ...
     * it, and one does: `memory` asks the answering model to close an answered
     ...
     *
     * Optional, like `useProfile`: four strategies never touch it, and a
     * strategy that returns nothing here keeps the reply exactly as the
     * provider sent it. The Agent still does not know what was taken out or
     * why — only that the strategy is the one allowed to say.
     */
